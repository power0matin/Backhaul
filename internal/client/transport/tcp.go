...
func (c *TcpTransport) Start() {
	...
	c.usageMonitor.SetTunnelStatus("Disconnected (TCP)")
...
func (c *TcpTransport) Restart() {
	c.restartOnce.Do(func() {
		...
		next := NewTCPClient(c.parentctx, c.config, c.logger)
	...
	for {
		...
		default:
			...
			if utils.SecureTokenEqual(message, c.config.Token) {
				...
				c.usageMonitor.SetTunnelStatus("Connected (TCP)")
				go c.poolMaintainer()
				go c.channelHandler()
...
func (c *TcpTransport) tunnelDialer() {
	...
	stopClose := context.AfterFunc(c.ctx, func() { _ = tcpConn.Close() })
	defer stopClose()
	defer tcpConn.Close()
	...
	case utils.SG_UDP:
		UDPDialer(c.ctx, tcpConn, resolvedAddr, c.logger, c.usageMonitor, port, c.config.Sniffer)
